...
public class SkystoneDetector  extends OpenCVPipeline {
    ...
    double onVal = 120.0;
    double oneCorrectPercent, twoCorrectPercent;
    ...
    enum Positions {unknown, one, two, three} //Indicates the position of the skystone

    Positions position = Positions.unknown;

    //The defaults are all wrong/copied from last year
    public SkystoneDetector(){
        super();
        SafeJsonReader reader = new SafeJsonReader("Vision");
    ...
    public Mat processFrame(Mat rgba, Mat gray) {
        ...
        if(oneSky) leftColor =  blue; else leftColor = red;
        if(twoSky) rightColor= blue; else rightColor= red;
    ...
    // looks at processed frame, to get location of block
    public void findGold(Mat input){
        ...
        oneCorrectPercent = ((double)correctCounter) / ((double)(counter));
        Log.i(TAG, "leftSelection: got " +correctCounter +" hits out of " + counter + " fraction: " + oneCorrectPercent);
        ...
        twoCorrectPercent = ((double)correctCounter) / ((double)(counter));
        Log.i(TAG, "rightSelection: got " +correctCounter +" hits out of " + counter + " fraction: " + twoCorrectPercent);

        oneSky = oneCorrectPercent > minDetectionThreshold;
        twoSky = twoCorrectPercent > minDetectionThreshold;
        ...
        if(oneSky && twoSky){
            ...
            if(oneCorrectPercent > twoCorrectPercent) position = Positions.one;
            else if(oneCorrectPercent < twoCorrectPercent) position = Positions.two;
        } // otherwise look at which is / isnt true
        else if (oneSky) position = Positions.one;
        else if (twoSky) position = Positions.two;
